...
int main ( int argc, char** argv )
{
    ...
    cv::viz::WCoordinateSystem world_coor(1.0),cameara_coor(0.5);
    cv::Point3d cam_pos (0,-1,-1) ,cam_focal_point(0,0,0),cam_y_dir(0,1,0);
    cv::Affine3d cam_pose = cv::viz::makeCameraPose(cam_pos,cam_focal_point,cam_y_dir);
    vis.setViewerPose(cam_pose);
    world_coor.setRenderingProperty(cv::viz::LINE_WIDTH,2.0);
    cameara_coor.setRenderingProperty(cv::viz::LINE_WIDTH,1.0);
    vis.showWidget("World",world_coor);
    vis.showWidget("Camara",cameara_coor);
    cout<<"read total"<<rgb_files.size()<<" endtries"<<endl;
